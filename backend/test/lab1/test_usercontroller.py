...
class TestGetUserByEmail:
    def test_valid_email_one_user(self, controller, mock_dao):
    ...
    def test_invalid_email(self, controller, mock_dao):
        mock_dao.find.side_effect = ValueError("Error: invalid email address")
        with pytest.raises(ValueError) as exc:
            controller.get_user_by_email("user")
        assert str(exc.value) == "Error: invalid email address"
    
    def test_valid_email_multiple_users(self, controller, mock_dao, capsys):
    ...
    def test_valid_email_no_users_found(self, controller, mock_dao):
    ...
    def test_trigger_exception_from_db(self, controller, mock_dao):
        ...
        assert  "DB Failure" == str(exc.value)
